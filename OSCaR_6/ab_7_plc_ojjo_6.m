function [drowh, dpileh]=ab_7_plc_ojjo_6(const, surface, dpileh, drowh)
...
%calculate cott height above ground
dpileh.cotth=dpileh.tpzc-dpileh.bpzc;
...
%calculate the wp height abov ground
lt= {mpts,                tpts}; %motor and truss points
wpv={geo.cott_wp_vert_mo, geo.cott_wp_vert_tr}; %motor and truss cott to wp
for i=1:numel(lt)
    dpileh.wpzc_oj(lt{i})=dpileh.tpzc(lt{i})+wpv{i}*m2f; %wp elevation z
    dpileh.wphc_oj(lt{i})=dpileh.cotth(lt{i})+wpv{i}*m2f;  %wp height
end
%calculate the y location of theoretical pile entry
for i=1:height(drowh)
    for j=drowh.si(i):drowh.ei(i)
        dpileh.bpyc_oj(j)=dpileh.tpyc(j)+sind(drowh.slp(i))*dpileh.wphc_oj(j);
    end
end
...
wpv={geo.cott_wp_vert_mo, geo.cott_wp_vert_tr}; %motor and truss cott to wp
for i=1:numel(lt)
    dpileh.xwl_oj(lt{i})=dpileh.tpxc(lt{i})-tand(ang{i})*dpileh.wphc_oj(lt{i}); %in feet
    dpileh.ywl_oj(lt{i})=dpileh.bpyc_oj(lt{i}); %legs are tangent to plane with slope of row
    dpileh.zwl_oj(lt{i})=surface.F_og(dpileh.xwl_oj(lt{i}),dpileh.ywl_oj(lt{i})); %surface at leg entry
    dpileh.xel_oj(lt{i})=dpileh.tpxc(lt{i})+tand(ang{i})*dpileh.wphc_oj(lt{i}); %in feet
    dpileh.yel_oj(lt{i})=dpileh.bpyc_oj(lt{i}); %legs are tangent to plane with slope of row
    dpileh.zel_oj(lt{i})=surface.F_og(dpileh.xel_oj(lt{i}),dpileh.yel_oj(lt{i})); %surface at leg entry
end

%find leg lengths
wp=[dpileh.tpxc,dpileh.tpyc,dpileh.wpzc_oj];%work point top
we=[dpileh.xwl_oj,dpileh.ywl_oj,dpileh.zwl_oj];%west entry point
ee=[dpileh.xel_oj,dpileh.yel_oj,dpileh.zel_oj];%east entry point
for i=1:height(wp)
...
for i=1:numel(tt)
    dpileh.wll_oj(tt{i}) = rwl(tt{i})-wpl{i}*m2f-rv{i}*m2f;  %west leg length in feet
    dpileh.ell_oj(tt{i}) = rel(tt{i})-wpl{i}*m2f-rv{i}*m2f; %east leg length in feet
end
dpileh.wllm_oj=dpileh.wll_oj./m2f;
dpileh.ellm_oj=dpileh.ell_oj./m2f;
dpileh.pile_rev=dpileh.cotth;
%discretize and roll up leg data
lf=const.leg_factory{1};
[X]=discretize(dpileh.wllm_oj,lf.lpbin);
[Y]=discretize(dpileh.ellm_oj,lf.lpbin);

for j=1:length(dpileh.wllm_oj)
    dpileh.LBW_oj(j)=lf.lpbin(X(j)+1);
    dpileh.LBE_oj(j)=lf.lpbin(Y(j)+1);
end
...
for j=2:length(lf.lpbin)
    lfout.westleg(j-1,1)=numel(find(abs(dpileh.LBW_oj-lf.lpbin(j))<.05));
    lfout.eastleg(j-1,1)=numel(find(abs(dpileh.LBE_oj-lf.lpbin(j))<.05));
end
...
writetable(lfout,append(const.dpath{1},'/legtable.xlsx'))
