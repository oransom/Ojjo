function [surface, dpileh] = ae_1_grading_6p1(const,surface,dpileh,drowh)
...
c2fr=1.1*cf;%grading.cut/grading.fill - set to run at least once
cfsso=0.05; %initial cut fill step size
cfss=cfsso; %for first step
iter=0;

while c2fr<cf-0.02 || c2fr>cf+0.02
    iter=iter+1;
    ...
    cfss_iter(iter)=abs(c2fr-cf);
    cfdiff(iter)=abs(c2fro-c2fr); 
    if iter>1 %this is a bunch of shit to just try and get it to converge - could be written a whole lot better - Newton Raphson?      
        if cfss_iter(iter)<0.7
            cfss=0.5*cfsso;
        elseif cfss_iter(iter)<0.6
            cfss=0.4*cfsso;
        elseif cfss_iter(iter)<0.5
            cfss=0.3*cfsso;
        elseif cfss_iter(iter)<0.4
            cfss=0.2*cfsso;
        elseif cfss_iter(iter)<0.3
            cfss=0.1*cfsso;
        elseif cfss_iter(iter)<0.2
            cfss=0.05*cfsso;
        elseif cfss_iter(iter)<0.1
            cfss=0.02*cfsso;
        else
            cfss=cfsso;
        end
    ...
    if iter>10
        if cfss_iter(iter)>cfss_iter(iter-1)
            cfsso=cfsso/2;
        end
        if abs(cfdiff(iter)-cfdiff(iter-1))<0.01
            cfsso=cfsso/2;
        end
    end
    if abs(c2fro-c2fr)<0.005
        break
    end
    for i=1:height(tar)
       if c2fr>cf %if more cut than fill
