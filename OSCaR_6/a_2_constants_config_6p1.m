function [const_all] = a_2_constants_config_6p1(config_file)
...
for k=1:height(const)
    ...
    mkdir(append(outpath,'/out/', string(const.project{k}), '_', string(datetime("today")),'/figures'));
    mkdir(append(outpath,'/out/', string(const.project{k}), '_', string(datetime("today")),'/data'));
    mkdir(append(outpath,'/out/', string(datetime("today")),'/rollups'));
    mkdir(append(outpath,'/out/', string(const.project{k}), '_', string(datetime("today")),'/grading'));
    const.fpath{k}=append(outpath,'/out/', string(const.project{k}), '_', string(datetime("today")),'/figures');
    const.dpath{k}=append(outpath,'/out/', string(const.project{k}), '_', string(datetime("today")),'/data');
    const.rupath{k}=append(outpath,'/out/', string(datetime("today")),'/rollups');
    const.gpath{k}=append(outpath,'/out/', string(const.project{k}), '_', string(datetime("today")),'/grading');
