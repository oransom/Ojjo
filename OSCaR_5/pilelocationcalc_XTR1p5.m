function [cpl, uc, trs, pmd, oos_data, ns_ugly] = pilelocationcalc_xtr(F_og,Flood_s,kpv,dat,span,const,surface)
...
for i=1:length(kpv)
    ...
    cpl.(kpv{i}).XTRjoint(abs(cpl.(kpv{i}).sumslope)<=1.5)=1;
    cpl.(kpv{i}).XTRjoint(abs(cpl.(kpv{i}).sumslope)>1.5)=2;
    ...
    y=cpl.(kpv{i}).tpy;
    z=cpl.(kpv{i}).tpz;
    cpl.(kpv{i}).sumslopec=2;
    xtrj=cpl.(kpv{i}).XTRjoint;
    while max(max(abs(cpl.(kpv{i}).sumslopec)>1.55))
        for j=1:m
            for k=1:last_s(j)
                if xtrj(j,k)==2
                    if k==1
                ...
                %cpl.(kpv{i}).tpz(j,k)=z(j,k);
            end
        end
        %tpzxtr=z-F_og(cpl.(kpv{i}).tpx,cpl.(kpv{i}).tpy);
        cpl.(kpv{i}).tpz_xtr=z;
        ...
        for j=1:m
            for k=1:last_s(j)
                if k==1
                    ...
                    cpl.(kpv{i}).sumslopec(j,k)=1.5;%cpl.(kpv{i}).slopedc(j,k)-cpl.(kpv{i}).slopeuc(j,k);
                    %above line for slope is minus because that makes sense w/
                    %passthrough slope
                elseif k==last_s(j)
        ...
        xtrj(abs(cpl.(kpv{i}).sumslopec)<=1.5)=1;
        xtrj(abs(cpl.(kpv{i}).sumslopec)>1.5)=2;
    end
