function [rnbr_table, mover_table, flipex] = rowmover_test4_nxt_ext(const, ot)
...
for i=1:length(rnbr.w)
    ...
    if rnbr.wpRemain(i)>0
     if rnbr.tpzc(i)-rnbr.tpzc(rnbr.w(i))<-rnbr.dw_ex_al(i) %if tpz of crow is greater than const.row_delta of west neighbor row
         rnbr.movecr(i)=1; %binary 1/0 for out of spec
         rnbr.d_west(i)=rnbr.tpzc(i)-rnbr.tpzc(rnbr.w(i)); %delta between rows that violate
         rnbr.m_west(i)=abs(rnbr.d_west(i))-rnbr.dw_ex_al(i);
...
for i=1:length(rnbr.e)
    if ~isnan(rnbr.e(i))
    if rnbr.wpRemain(i)>0
     if rnbr.tpzc(i)-rnbr.tpzc(rnbr.e(i))<-rnbr.de_ex_al(i) %if tpz of crow is greater than const.row_delta of east neighbor row
         rnbr.movecr(i)=1;
         rnbr.d_east(i)=rnbr.tpzc(i)-rnbr.tpzc(rnbr.e(i));
         rnbr.m_east(i)=abs(rnbr.d_east(i))-rnbr.de_ex_al(i);
...
for i=1:length(rnbr.movecr)
    if rnbr.movecr(i)==1 && rnbr.d_west(i) < -rnbr.dw_ex_al(i) %check if current row is triggering and then flip the next row over
        if ~isnan(rnbr.w(i))
        rnbr.flipexterior(rnbr.w(i))=1;
        end
        if ~isnan(rnbr.w(rnbr.w(i)))
        if rnbr.tpzc(rnbr.w(rnbr.w(i)))-rnbr.tpzc(i)>rnbr.dw_ex_al(i) %look two rows over to see if we need to flip it as well based on tpzc there vs. center
            rnbr.flipexterior(rnbr.w(rnbr.w(i)))=1;
        end
        end
    elseif rnbr.movecr(i)==1 && rnbr.d_east(i) < -rnbr.de_ex_al(i)
        if ~isnan(rnbr.e(i))
        rnbr.flipexterior(rnbr.e(i))=1;
        end
        if ~isnan(rnbr.e(rnbr.e(i)))
        if rnbr.tpzc(rnbr.e(rnbr.e(i)))-rnbr.tpzc(i)>rnbr.de_ex_al(i)
