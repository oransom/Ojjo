function [rnbr_table, mover_table, flipex] = rowmover_test4(const, ot)
...
for i=1:length(rnbr.tpzc)
    if ~isnan(rnbr.e(i))
        rnbr.slp(i)=atand((rnbr.tpzc(i)-rnbr.tpzc(rnbr.e(i)))/neighbor_distance);
    end
end
...
for i=1:length(rnbr.w)
    ...
    if rnbr.wpRemain(i)>0
     if rnbr.tpzc(i)-rnbr.tpzc(rnbr.w(i))<-const.row_delta %if tpz of crow is greater than const.row_delta of west neighbor row
         rnbr.movecr(i)=1; %binary 1/0 for out of spec
         rnbr.d_west(i)=rnbr.tpzc(i)-rnbr.tpzc(rnbr.w(i)); %delta between rows that violate
         rnbr.m_west(i)=abs(rnbr.d_west(i))-const.row_delta+ewpad;
...
for i=1:length(rnbr.e)
    if ~isnan(rnbr.e(i))
    if rnbr.wpRemain(i)>0
     if rnbr.tpzc(i)-rnbr.tpzc(rnbr.e(i))<-const.row_delta %if tpz of crow is greater than const.row_delta of east neighbor row
         rnbr.movecr(i)=1;
         rnbr.d_east(i)=rnbr.tpzc(i)-rnbr.tpzc(rnbr.e(i));
         rnbr.m_east(i)=abs(rnbr.d_east(i))-const.row_delta+ewpad;
...
for i=1:length(rnbr.movecr)
    if rnbr.movecr(i)==1 && rnbr.d_west(i) < -const.row_delta %check if current row is triggering and then flip the next row over
        if ~isnan(rnbr.w(i))
        rnbr.flipexterior(rnbr.w(i))=1;
        end
        if ~isnan(rnbr.w(rnbr.w(i)))
        if rnbr.tpzc(rnbr.w(rnbr.w(i)))-rnbr.tpzc(i)>const.row_delta %look two rows over to see if we need to flip it as well based on tpzc there vs. center
            rnbr.flipexterior(rnbr.w(rnbr.w(i)))=1;
        end
        end
    elseif rnbr.movecr(i)==1 && rnbr.d_east(i) < -const.row_delta
        if ~isnan(rnbr.e(i))
        rnbr.flipexterior(rnbr.e(i))=1;
        end
        if ~isnan(rnbr.e(rnbr.e(i)))
        if rnbr.tpzc(rnbr.e(rnbr.e(i)))-rnbr.tpzc(i)>const.row_delta
...
flipex.bsr=rnbr.bsrc; flipex.tpxc=rnbr.tpxc; flipex.tpyc=rnbr.tpyc; flipex.tpzc=rnbr.tpzc;
flipex.row_len=rnbr.row_length;
flipex.flip=rnbr.flipexterior;
flipex.flip(flipex.flip==0)=NaN;
flipex=struct2table(flipex);
flipex=rmmissing(flipex);
...
if const.writefiles==1 && const.simple_out==1
    mkdir([const.outpath '/flipex' ]);
    writetable(flipex,[const.outpath '/flipex/' char(const.sect_name) ' flipex.csv'])
end
